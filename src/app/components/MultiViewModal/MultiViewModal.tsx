...
import {
  hasApplied,
  makeApplicationID,
} from "@/app/server/services/ApplicationService";
...
const MultiViewModal: React.FC<Props> = ({ job, modalIsOpen, onClose }) => {
  ...
  useEffect(() => {
    if (modalIsOpen) {
      ...
      setViewData({
        ...
        company: job.jobData.title,
        description: job.jobData.description,
        minBudget: job.jobData.budgetMin,
        maxBudget: job.jobData.budgetMax,
  ...
  const ViewB = () => {
    if (formData) {
      return <JobForm data={formData} onClose={onClose} />;
    }
  };
